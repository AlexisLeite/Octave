...
const IMPLICIT_NOTEBOOK_SOURCES = [
  ...
  },
  {
    label: "print-double",
    code: [
      "function printDouble(title_or_x, x)",
      "  if nargin == 1",
      "    x = title_or_x;",
      '    title = "";',
      "  elseif nargin == 2",
      "    title = title_or_x;",
      "    if isstring(title) && isscalar(title)",
      "      title = char(title);",
      "    endif",
      "    if ! ischar(title) || rows(title) != 1",
      '      error("printDouble: el titulo debe ser texto");',
      "    endif",
      "  else",
      '    error("printDouble: use printDouble(x) o printDouble(titulo, x)");',
      "  endif",
      '  if ! isa(x, "double") || ! isreal(x) || ! isscalar(x)',
      '    error("printDouble: x debe ser un escalar real de tipo double");',
      "  endif",
      '  bits = dec2bin(typecast(x, "uint64"), 64);',
      "  signo = bits(1);",
      "  exponente = bits(2:12);",
      "  mantisa = bits(13:64);",
      "  mandec = bin2dec(mantisa);",
      "  expbitsdec = bin2dec(exponente);",
      "  if expbitsdec == 2047",
      "    if mandec != 0",
      '      significado = "NaN";',
      '    elseif signo == "0"',
      ...
      "    endif",
      "    if isempty(title)",
      '      printf("%-5s %-3s %-13s %s\\n", significado, signo, exponente, mantisa);',
      "    else",
      '      printf("%s    %-5s %-3s %-13s %s\\n", title, significado, signo, exponente, mantisa);',
      "    endif",
      "    return",
      "  elseif expbitsdec == 0",
      "    expdec = -1022;",
      "  else",
      "    expdec = expbitsdec - 1023;",
      "  endif",
      "  if isempty(title)",
      '    printf("%-3s %-13s (%5d)   %-54s (%20d)\\n", signo, exponente, expdec, mantisa, mandec);',
      "  else",
      '    printf("%s    %-3s %-13s (%5d)   %-54s (%20d)\\n", title, signo, exponente, expdec, mantisa, mandec);',
      "  endif",
      "endfunction",
    ].join("\n"),
  },
  {
    label: "print-single",
    code: [
      "function printSingle(title_or_x, x)",
      "  if nargin == 1",
      "    x = title_or_x;",
      '    title = "";',
      "  elseif nargin == 2",
      ...
      "    endif",
      "    if ! ischar(title) || rows(title) != 1",
      '      error("printSingle: el titulo debe ser texto");',
      "    endif",
      "  else",
      '    error("printSingle: use printSingle(x) o printSingle(titulo, x)");',
      "  endif",
      '  if ! isa(x, "single") || ! isreal(x) || ! isscalar(x)',
      '    error("printSingle: x debe ser un escalar real de tipo single");',
      "  endif",
      ...
      '      printf("%-5s %-3s %-10s %s\\n", significado, signo, exponente, mantisa);',
      "    else",
      '      printf("%s    %-5s %-3s %-10s %s\\n", title, significado, signo, exponente, mantisa);',
      "    endif",
      "    return",
      "  elseif expbitsdec == 0",
      "    expdec = -126;",
      "  else",
      "    expdec = expbitsdec - 127;",
      "  endif",
      "  if isempty(title)",
      '    printf("%-3s %-10s (%5d)   %-25s (%10d)\\n", signo, exponente, expdec, mantisa, mandec);',
      "  else",
      '    printf("%s    %-3s %-10s (%5d)   %-25s (%10d)\\n", title, signo, exponente, expdec, mantisa, mandec);',
      "  endif",
      "endfunction",
    ].join("\n"),
  },
...
class OctaveRuntime {
  ...
  static async create(
    ...
    try {
      ...
      for (const source of IMPLICIT_NOTEBOOK_SOURCES) {
        await runtime.enqueue(() => runtime.runSource(source.code, `startup-${source.label}`));
      }
      return runtime;
    } catch (error) {
