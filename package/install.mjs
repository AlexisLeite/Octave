...
// scripts/install-template.mjs
var packageDirectory = path2.dirname(fileURLToPath(import.meta.url));
var program2 = new Command().name("install.mjs").description("Instala Octave Notebook desde el paquete npm contiguo").version("0.1.0").option("--prefix <directorio>", "Directorio de instalaci\xF3n").option("--force", "Permite instalar en un directorio no vac\xEDo").showHelpAfterError();
program2.parse();
await install(program2.opts()).catch((error) => {
...
async function install(options) {
  ...
  let prefix = options.prefix ? path2.resolve(options.prefix) : null;
  ...
  try {
    if (!prefix) {
      ...
      const response = await ask(answers, `Directorio de instalaci\xF3n [${fallback}]: `);
      prefix = path2.resolve(response.trim() || fallback);
    }
    const nonEmpty = existsSync(prefix) && readdirSync(prefix).length > 0;
    if (nonEmpty && !options.force) {
      if (!interactive) throw new Error("El directorio de instalaci\xF3n no est\xE1 vac\xEDo. Use --force para continuar.");
      const confirmation = await ask(answers, "El directorio no est\xE1 vac\xEDo. \xBFContinuar? [y/N]: ");
      if (!/^y(?:es)?$|^s(?:í|i)?$/i.test(confirmation.trim())) throw new Error("Instalaci\xF3n cancelada.");
    }
    const archive = path2.join(packageDirectory, archives[0]);
...
function defaultPrefix() {
  if (process.platform === "win32") return path2.join(process.env.LOCALAPPDATA || path2.join(homedir(), "AppData", "Local"), "Octave Notebook");
  return path2.join(homedir(), ".local", "share", "octave-notebook");
}
async function ask(readline, prompt) {
  if (process.stdin.destroyed) throw new Error("No hay entrada disponible para completar la instalaci\xF3n interactiva. Use --prefix.");
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 3e4);
  try {
    return await readline.question(prompt, { signal: controller.signal });
  } catch (error) {
    if (error?.name === "AbortError") throw new Error("Tiempo agotado esperando una respuesta. Use --prefix para instalar sin interacci\xF3n.");
    throw new Error("No se pudo leer la respuesta. Use --prefix para instalar sin interacci\xF3n.");
  } finally {
    clearTimeout(timer);
  }
}
function runNpm(args) {
  return new Promise((resolveRun, rejectRun) => {
    ...
    child.once("exit", (code) => code === 0 ? resolveRun() : rejectRun(new Error(`npm install termin\xF3 con c\xF3digo ${code}.`)));
  });
}
